...
router.get('/me', auth, async (req, res) => {
  try {
    ...
    const result = await db.query(
      'SELECT employee_id, full_name, department, designation, email, date_of_joining, status FROM employees WHERE employee_id = $1',
...
router.post('/', auth, requireRole(['HR', 'Admin']), async (req, res) => {
  try {
    ...
    const existingEmail = await db.query('SELECT email FROM employees WHERE email = $1', [email]);
    ...
    const employee_id = `EMP${String(currentCount + 1).padStart(4, '0')}`;

    await db.query(
      'INSERT INTO employees (employee_id, full_name, department, designation, email, date_of_joining, status) VALUES ($1, $2, $3, $4, $5, $6, $7)',
...
router.put('/:employee_id', auth, requireRole(['HR', 'Admin']), async (req, res) => {
  try {
    ...
    const existingEmail = await db.query(
      'SELECT email FROM employees WHERE email = $1 AND employee_id != $2',
      [email, employee_id]
    );
    if (existingEmail.rows.length > 0) {
      return res.status(400).json({ detail: 'Employee with this email already exists' });
    ...
    const result = await db.query(
      'UPDATE employees SET full_name = $1, department = $2, designation = $3, email = $4, date_of_joining = $5, status = $6 WHERE employee_id = $7',
...
router.delete('/:employee_id', auth, requireRole(['HR', 'Admin']), async (req, res) => {
  try {
    ...
    const result = await db.query('DELETE FROM employees WHERE employee_id = $1', [employee_id]);
...
router.post('/import', auth, requireRole(['HR', 'Admin']), upload.single('file'), async (req, res) => {
  try {
    ...
    try {
      ...
      const initialCountResult = await client.query('SELECT COUNT(*) AS count FROM employees');
      let employeeCounter = Number(initialCountResult.rows[0]?.count) || 0;
      ...
      for (let i = 2; i <= actualRowCount; i++) {
        ...
        try {
          ...
          // Check if email already exists
          const existingEmail = await client.query('SELECT email FROM employees WHERE email = $1', [email]);
          if (existingEmail.rows.length > 0) {
